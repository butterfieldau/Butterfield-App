...
router.post('/stores', async (req, res) => {
  ...
  const {
    ...
    printerIp, printerPort = 9100, printerBrand = 'epson', autoPrint = true, autoDrawer = false, drawerPin = 0, orderCutoffTime, dailySpecial,
  ...
  const [store] = await db.insert(storesTable).values({
    ...
    printerIp, printerPort, printerBrand, autoPrint, autoDrawer, drawerPin, orderCutoffTime, dailySpecial, status,
...
router.patch('/stores/:id', requireManagerPermission('settings'), async (req, res) => {
  ...
  const allowed = [
    ...
    'latitude','longitude','geofenceRadius','phone','email','website','imageUrl',
    'printerIp','printerPort','printerBrand','autoPrint','autoDrawer','drawerPin','orderCutoffTime','dailySpecial',
