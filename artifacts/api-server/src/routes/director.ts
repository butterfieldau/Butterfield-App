    ...
    if (isCancelOrRefund && !wasAlreadyCancelledOrRefunded && updated) {
      ...
      try {
        ...
        if (previousStatus === 'completed') {
          ...
          req.log.info({ orderId: updated.id, userId: updated.userId, status, previousStatus, outstandingStampCount }, 'Director coffee stamp reversal check');
          ...
          req.log.info({ orderId: updated.id, userId: updated.userId, status, previousStatus }, 'Director coffee stamp reversal skipped — order was never completed');
        }
      } catch (err: any) {
...
router.patch('/wholesale/invoices/:orderId/mark-paid', async (req, res) => {
  ...
  try {
    ...
    if (recipientEmail) {
      ...
      import('../lib/invoiceEmailSender.js').then(async ({ sendPaymentReceivedEmail }) => {
        const paymentTermsMap: Record<string, string> = {
          pay_on_order: 'Pay on order', net_7: '7 days from invoice date',
          net_14: '14 days from invoice date', net_30: '30 days from invoice date',
          net_60: '60 days from invoice date',
...
router.post('/products', async (req, res) => {
  const {
    ...
    isAvailable, isFeatured, isNew, isWholesaleAvailable, isStaffOnly, isAppOnly, isPosOnly,
...
router.patch('/products/:id', async (req, res) => {
  ...
  const allowed = [
    ...
    'isAppOnly','isPosOnly','isLimitedDrop','isSoldOut','isComingSoon','isPickupOnly',
    'tags','allergens','dietaryTags','ingredients','nutritionInfo',
    'storageInstructions','servingInstructions',
    'minOrderQty','maxOrderQty','leadTimeMins','availableDays','availableTimes',
    'stockCount','lowStockThreshold','sortOrder',
