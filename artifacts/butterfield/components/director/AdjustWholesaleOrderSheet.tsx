...
const MEMO_TYPES = [
  { key: 'credit_memo',     label: 'Credit Memo',     icon: 'file-text',   color: BRAND },
    ...
    <Modal visible={visible} animationType="slide" presentationStyle="pageSheet" onRequestClose={onClose}>
      <View style={{ flex: 1, backgroundColor: BG }}>
        ...
        <ScrollView contentContainerStyle={{ padding: 16, gap: 14, paddingBottom: 40 }} keyboardShouldPersistTaps="handled">
          ...
          {/* Payment context */}
          <View style={{ backgroundColor: SURFACE, borderRadius: 14, padding: 14, gap: 8 }}>
            <Text style={{ fontSize: 12, fontWeight: '700', color: TEXT_MUTED, letterSpacing: 0.6 }}>ORDER PAYMENT</Text>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
              <Text style={{ color: TEXT_MUTED, fontSize: 13 }}>Order total</Text>
              <Text style={{ color: TEXT, fontWeight: '600', fontSize: 13 }}>${((order.totalCents ?? 0) / 100).toFixed(2)}</Text>
            </View>
            {(order.refundedCents ?? 0) > 0 && (
              <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                <Text style={{ color: TEXT_MUTED, fontSize: 13 }}>Already refunded</Text>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: isPaidByCard ? GREEN + '10' : AMBER + '10', borderRadius: 8, padding: 8 }}>
              <Feather name={isPaidByCard ? 'credit-card' : 'file-text'} size={13} color={isPaidByCard ? GREEN : AMBER} />
              <Text style={{ color: isPaidByCard ? GREEN : AMBER, fontSize: 12, fontWeight: '500', flex: 1 }}>
                {isPaidByCard
          ...
          {orderItems.length > 0 && (
            <View style={{ backgroundColor: SURFACE, borderRadius: 14, padding: 16 }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                <Text style={{ fontSize: 12, fontWeight: '700', color: TEXT_MUTED, letterSpacing: 0.6 }}>PER LINE ITEM (OPTIONAL)</Text>
                <Text style={{ fontSize: 10, color: TEXT_MUTED }}>Tap % to quick-fill</Text>
              </View>
                      ...
                      <View style={{ flexDirection: 'row', gap: 5 }}>
                        {[50, 100].map(pct => (
                          <Pressable
                            ...
                            style={{ paddingHorizontal: 7, paddingVertical: 4, borderRadius: 8, backgroundColor: BRAND_DIM, borderWidth: 1, borderColor: BRAND + '40' }}
                          >
                            <Text style={{ color: BRAND, fontSize: 10, fontWeight: '600' }}>{pct}%</Text>
                          </Pressable>
                        ))}
                      </View>
                    </View>
                    {/* Per-line refund input */}
                    <View style={{ flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderColor: lineCents > itemMax ? RED : (lineCents > 0 ? BRAND : BORDER), borderRadius: 8, paddingHorizontal: 10, paddingVertical: 7, backgroundColor: SURFACE_RAISED, gap: 4 }}>
              ...
              {lineTotal > 0 && (
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', paddingTop: 10, marginTop: 4, borderTopWidth: 1, borderTopColor: BORDER }}>
                  ...
                  <Text style={{ color: lineTotal > maxRefundable ? RED : BRAND, fontWeight: '700', fontSize: 14 }}>${(lineTotal / 100).toFixed(2)}</Text>
                </View>
              )}
            </View>
          )}
          ...
          {/* Memo type */}
          <View style={{ backgroundColor: SURFACE, borderRadius: 14, padding: 16 }}>
            <Text style={{ fontSize: 12, fontWeight: '700', color: TEXT_MUTED, letterSpacing: 0.6, marginBottom: 10 }}>TYPE</Text>
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
              {MEMO_TYPES.map(t => {
                const active = memoType === t.key;
                return (
                  <Pressable key={t.key} onPress={() => { setMemoType(t.key); Haptics.selectionAsync(); }}
                    style={{ flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 12, paddingVertical: 8, borderRadius: 20, borderWidth: 1, borderColor: active ? t.color : BORDER, backgroundColor: active ? `${t.color}20` : SURFACE_RAISED }}>
                    <Feather name={t.icon as any} size={13} color={active ? t.color : TEXT_MUTED} />
                    <Text style={{ color: active ? t.color : TEXT_MUTED, fontWeight: '600', fontSize: 12 }}>{t.label}</Text>
                  </Pressable>
                );
              })}
            </View>
          ...
          <View style={{ backgroundColor: SURFACE, borderRadius: 14, padding: 16 }}>
            <Text style={{ fontSize: 12, fontWeight: '700', color: TEXT_MUTED, letterSpacing: 0.6, marginBottom: 8 }}>
              TOTAL CREDIT AMOUNT (AUD)
              {lineTotal > 0 && !manualAmount ? <Text style={{ color: BRAND, fontWeight: '400' }}> — from line items</Text> : null}
            </Text>
            <View style={{ flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderColor: amountCents > maxRefundable ? RED : BORDER, borderRadius: 12, paddingHorizontal: 14, paddingVertical: 12, backgroundColor: SURFACE_RAISED }}>
              <Text style={{ color: TEXT_MUTED, fontSize: 18, marginRight: 4 }}>$</Text>
              <TextInput
              ...
              {[10, 25, 50].map(pct => {
                ...
                return (
                  <Pressable key={pct}
                    ...
                    style={{ flex: 1, paddingVertical: 7, borderRadius: 10, backgroundColor: BRAND_DIM, borderWidth: 1, borderColor: BRAND + '40', alignItems: 'center' }}>
                    <Text style={{ color: BRAND, fontWeight: '600', fontSize: 12 }}>{pct}%</Text>
                    <Text style={{ color: TEXT_MUTED, fontSize: 10 }}>${(pctCents / 100).toFixed(2)}</Text>
                  </Pressable>
                );
              })}
              <Pressable
                onPress={() => { setAmountAud((maxRefundable / 100).toFixed(2)); setManualAmount(true); setLineAmounts({}); Haptics.selectionAsync(); }}
                style={{ flex: 1, paddingVertical: 7, borderRadius: 10, backgroundColor: AMBER + '20', borderWidth: 1, borderColor: AMBER + '40', alignItems: 'center' }}>
                <Text style={{ color: AMBER, fontWeight: '600', fontSize: 12 }}>Full</Text>
                <Text style={{ color: TEXT_MUTED, fontSize: 10 }}>${(maxRefundable / 100).toFixed(2)}</Text>
              </Pressable>
            </View>
          </View>
          ...
          {/* Reason */}
          <View style={{ backgroundColor: SURFACE, borderRadius: 14, padding: 16 }}>
            <Text style={{ fontSize: 12, fontWeight: '700', color: TEXT_MUTED, letterSpacing: 0.6, marginBottom: 8 }}>REASON *</Text>
            <TextInput
              style={{ borderWidth: 1, borderColor: BORDER, borderRadius: 10, padding: 12, fontSize: 14, color: TEXT, minHeight: 80, textAlignVertical: 'top', backgroundColor: SURFACE_RAISED }}
          ...
          {existingMemos.length > 0 && (
            <View style={{ backgroundColor: SURFACE, borderRadius: 14, padding: 16 }}>
              <Text style={{ fontSize: 12, fontWeight: '700', color: TEXT_MUTED, letterSpacing: 0.6, marginBottom: 10 }}>EXISTING CREDITS</Text>
              {existingMemos.map((memo, i) => (
                <View key={i} style={{ paddingVertical: 8, borderBottomWidth: i < existingMemos.length - 1 ? 1 : 0, borderBottomColor: BORDER, gap: 2 }}>
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                    ...
                    <Text style={{ color: TEXT_MUTED, fontSize: 11 }}>{memo.createdAt ? new Date(memo.createdAt).toLocaleDateString('en-AU', { day: 'numeric', month: 'short', year: 'numeric' }) : ''}</Text>
                  </View>
                  <Text style={{ color: TEXT_MUTED, fontSize: 12 }}>{memo.reason ?? memo.type ?? 'Credit memo'} · by {memo.createdBy ?? '—'}</Text>
                  ...
                  {Array.isArray(memo.lineItems) && memo.lineItems.length > 0 && (
                    <View style={{ marginTop: 4, gap: 2 }}>
                      {memo.lineItems.map((li: any, j: number) => (
                        <Text key={j} style={{ color: TEXT_MUTED, fontSize: 11, paddingLeft: 8 }}>
        ...
        <View style={{ padding: 16, paddingBottom: insets.bottom + 8, backgroundColor: BG, borderTopWidth: 1, borderTopColor: BORDER }}>
          <Pressable
            ...
            style={{ height: 52, borderRadius: 14, backgroundColor: saving || !isValid ? TEXT_MUTED : RED, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 8 }}
          >
