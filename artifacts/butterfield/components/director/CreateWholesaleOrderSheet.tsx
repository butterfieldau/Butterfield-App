        ...
        return (
          <React.Fragment key={s}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
              <View style={{ width: 22, height: 22, borderRadius: 11, backgroundColor: done ? GREEN : active ? BRAND : SURFACE_RAISED, alignItems: 'center', justifyContent: 'center' }}>
                {done ? <Feather name="check" size={12} color={BRAND_TEXT_ON} /> : <Text style={{ color: active ? BRAND_TEXT_ON : TEXT_MUTED, fontSize: 11, fontWeight: '700' }}>{idx + 1}</Text>}
              </View>
              <Text style={{ fontSize: 11, fontWeight: active ? '700' : '400', color: active ? BRAND : TEXT_MUTED }}>
      ...
      <View style={{ flex: 1, backgroundColor: BG }}>
        ...
        {step === 'account' && (
          <View style={{ flex: 1 }}>
            <View style={{ padding: 14, paddingBottom: 8 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: SURFACE, borderRadius: 12, borderWidth: 1, borderColor: BORDER, paddingHorizontal: 12, paddingVertical: 8, gap: 8 }}>
            ...
            <FlatList
              ...
              ListEmptyComponent={<View style={{ alignItems: 'center', marginTop: 40 }}><Feather name="users" size={32} color={BORDER} /><Text style={{ color: TEXT_MUTED, marginTop: 10, fontSize: 14 }}>No approved accounts found</Text></View>}
              renderItem={({ item: acct }) => (
                <Pressable
                  ...
                  style={{ backgroundColor: SURFACE, borderRadius: 12, padding: 14, borderWidth: 1, borderColor: accountId === acct.id ? BRAND : BORDER, flexDirection: 'row', alignItems: 'center', gap: 12 }}>
                  <View style={{ width: 38, height: 38, borderRadius: 19, backgroundColor: BRAND_DIM, alignItems: 'center', justifyContent: 'center' }}>
                    <Text style={{ color: BRAND, fontWeight: '700', fontSize: 13 }}>{(acct.companyName ?? '?').slice(0, 2).toUpperCase()}</Text>
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text style={{ color: TEXT, fontWeight: '600', fontSize: 14 }}>{acct.companyName ?? acct.email}</Text>
                    <Text style={{ color: TEXT_MUTED, fontSize: 12 }}>{acct.pricingTier ?? 'Standard'} · {acct.abn ? `ABN ${acct.abn}` : 'No ABN'}</Text>
                  </View>
            ...
            {accountId && (
              <View style={{ padding: 14, paddingTop: 8, borderTopWidth: 1, borderTopColor: BORDER }}>
                <Pressable
                  ...
                  style={{ height: 48, backgroundColor: BRAND, borderRadius: 12, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 8 }}
                >
                  <Text style={{ color: BRAND_TEXT_ON, fontWeight: '700', fontSize: 15 }}>Continue</Text>
                  <Feather name="arrow-right" size={16} color={BRAND_TEXT_ON} />
                </Pressable>
              </View>
            )}
          </View>
          ...
          <View style={{ flex: 1 }}>
            {selectedAccount && (
              <Pressable onPress={() => setStep('account')}
                style={{ flexDirection: 'row', alignItems: 'center', gap: 8, padding: 12, paddingHorizontal: 16, backgroundColor: SURFACE_RAISED, borderBottomWidth: 1, borderBottomColor: BORDER }}>
                <Feather name="briefcase" size={14} color={BRAND} />
                <Text style={{ color: BRAND, fontWeight: '600', fontSize: 13 }}>{selectedAccount.companyName}</Text>
                <Feather name="chevron-down" size={13} color={BRAND} />
              </Pressable>
            )}
            {/* Product search bar */}
            ...
            <FlatList
              ...
              ListEmptyComponent={
                <View style={{ alignItems: 'center', paddingTop: 40 }}>
                  ...
                  <Text style={{ color: TEXT_MUTED, marginTop: 10, fontSize: 14 }}>
              ...
              renderItem={({ item: product }) => {
                ...
                const unitCents = selectedTierDiscountPct > 0
                  ...
                  : baseCents;
                return (
                  <View style={{ backgroundColor: SURFACE, borderRadius: 12, padding: 12, flexDirection: 'row', alignItems: 'center', gap: 10, borderWidth: 1, borderColor: cartItem ? BRAND : BORDER }}>
                    <View style={{ flex: 1 }}>
                      ...
                      <Text style={{ color: TEXT_MUTED, fontSize: 12 }}>
                        ...
                        {selectedTierDiscountPct > 0 && (
                          <Text style={{ color: BRAND }}> ({selectedTierDiscountPct}% tier)</Text>
                        )}
                      </Text>
                    </View>
                    {cartItem ? (
                      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                        <Pressable onPress={() => updateQty(cart.indexOf(cartItem), -1)} style={{ width: 28, height: 28, borderRadius: 14, backgroundColor: cartItem.qty === 1 ? RED + '20' : SURFACE_RAISED, alignItems: 'center', justifyContent: 'center' }}>
                        ...
                        <Pressable onPress={() => addToCart(product)} style={{ width: 28, height: 28, borderRadius: 14, backgroundColor: BRAND, alignItems: 'center', justifyContent: 'center' }}>
                          <Feather name="plus" size={13} color={BRAND_TEXT_ON} />
                        </Pressable>
                      </View>
                    ) : (
                      <Pressable
                        ...
                        accessibilityRole="button"
                        style={{ width: 34, height: 34, borderRadius: 17, backgroundColor: BRAND_DIM, borderWidth: 1, borderColor: BRAND, alignItems: 'center', justifyContent: 'center' }}
                      >
            ...
            {cart.length > 0 && (
              <View style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: 16, paddingBottom: insets.bottom + 8, backgroundColor: BG, borderTopWidth: 1, borderTopColor: BORDER }}>
                <Pressable onPress={() => setStep('summary')}
                  style={{ height: 52, borderRadius: 14, backgroundColor: BRAND, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 8 }}>
                  <Feather name="arrow-right" size={18} color={BRAND_TEXT_ON} />
                  <Text style={{ color: BRAND_TEXT_ON, fontWeight: '700', fontSize: 16 }}>Review Order ({cart.length} items · ${(totalCents / 100).toFixed(2)})</Text>
                </Pressable>
              </View>
            )}
          </View>
        ...
        {step === 'summary' && (
          <ScrollView contentContainerStyle={{ padding: 16, gap: 14, paddingBottom: 100 }} keyboardShouldPersistTaps="handled">
            {/* Account summary */}
            {selectedAccount && (
              <View style={{ backgroundColor: SURFACE, borderRadius: 14, padding: 14, flexDirection: 'row', alignItems: 'center', gap: 12 }}>
                <View style={{ width: 40, height: 40, borderRadius: 20, backgroundColor: BRAND_DIM, alignItems: 'center', justifyContent: 'center' }}>
                  <Text style={{ color: BRAND, fontWeight: '700', fontSize: 14 }}>{(selectedAccount.companyName ?? '?').slice(0, 2).toUpperCase()}</Text>
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={{ color: TEXT, fontWeight: '700', fontSize: 15 }}>{selectedAccount.companyName}</Text>
                  <Text style={{ color: TEXT_MUTED, fontSize: 12 }}>{selectedAccount.pricingTier ?? 'Standard'}</Text>
                </View>
                <Pressable onPress={() => setStep('account')} style={{ padding: 6 }}>
                  <Feather name="edit-2" size={15} color={BRAND} />
                </Pressable>
              </View>
            )}

            {/* Items summary */}
            <View style={{ backgroundColor: SURFACE, borderRadius: 14, padding: 14 }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 10 }}>
                <Text style={{ fontSize: 12, fontWeight: '700', color: TEXT_MUTED, letterSpacing: 0.6 }}>ITEMS</Text>
                <Pressable onPress={() => setStep('items')}><Text style={{ color: BRAND, fontSize: 12, fontWeight: '600' }}>Edit</Text></Pressable>
              </View>
              {cart.map((item, i) => (
                <View key={i} style={{ flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 8, borderBottomWidth: i < cart.length - 1 ? 1 : 0, borderBottomColor: BORDER }}>
                  ...
                  <Text style={{ color: TEXT_MUTED, fontSize: 13 }}>${((item.qty * item.unitPriceCents) / 100).toFixed(2)}</Text>
                </View>
              ))}
              ...
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', paddingTop: 10, marginTop: 4, borderTopWidth: 1, borderTopColor: BORDER }}>
                ...
                <Text style={{ color: BRAND, fontWeight: '700', fontSize: 17 }}>AUD ${(totalCents / 100).toFixed(2)}</Text>
              </View>
            </View>

            {/* Delivery type */}
            <View style={{ backgroundColor: SURFACE, borderRadius: 14, padding: 14 }}>
              <Text style={{ fontSize: 12, fontWeight: '700', color: TEXT_MUTED, letterSpacing: 0.6, marginBottom: 10 }}>FULFILMENT</Text>
              <View style={{ flexDirection: 'row', gap: 10 }}>
                {(['pickup', 'delivery'] as const).map(dt => (
                  <Pressable key={dt} onPress={() => { setDeliveryType(dt); Haptics.selectionAsync(); }}
                    style={{ flex: 1, flexDirection: 'row', alignItems: 'center', gap: 8, padding: 12, borderRadius: 12, borderWidth: 2, borderColor: deliveryType === dt ? BRAND : BORDER, backgroundColor: deliveryType === dt ? BRAND_DIM : SURFACE_RAISED }}>
                    <Feather name={dt === 'pickup' ? 'shopping-bag' : 'truck'} size={16} color={deliveryType === dt ? BRAND : TEXT_MUTED} />
                    <Text style={{ color: deliveryType === dt ? BRAND : TEXT_MUTED, fontWeight: '700', fontSize: 13, textTransform: 'capitalize' }}>{dt}</Text>
                  </Pressable>
                ))}
              </View>
            </View>
            ...
            {/* Scheduled date */}
            <View style={{ backgroundColor: SURFACE, borderRadius: 14, padding: 14 }}>
              <Text style={{ fontSize: 12, fontWeight: '700', color: TEXT_MUTED, letterSpacing: 0.6, marginBottom: 8 }}>SCHEDULED DELIVERY DATE (OPTIONAL)</Text>
              <TextInput
                style={{ borderWidth: 1, borderColor: BORDER, borderRadius: 10, padding: 12, fontSize: 14, color: TEXT, backgroundColor: SURFACE_RAISED }}
            ...
            {/* PO reference */}
            <View style={{ backgroundColor: SURFACE, borderRadius: 14, padding: 14 }}>
              <Text style={{ fontSize: 12, fontWeight: '700', color: TEXT_MUTED, letterSpacing: 0.6, marginBottom: 8 }}>PO REFERENCE</Text>
              <TextInput
                style={{ borderWidth: 1, borderColor: BORDER, borderRadius: 10, padding: 12, fontSize: 14, color: TEXT, backgroundColor: SURFACE_RAISED }}
            ...
            {/* Notes */}
            <View style={{ backgroundColor: SURFACE, borderRadius: 14, padding: 14 }}>
              <Text style={{ fontSize: 12, fontWeight: '700', color: TEXT_MUTED, letterSpacing: 0.6, marginBottom: 8 }}>NOTES (OPTIONAL)</Text>
              <TextInput
        ...
        {step === 'summary' && (
          <View style={{ padding: 16, paddingBottom: insets.bottom + 8, backgroundColor: BG, borderTopWidth: 1, borderTopColor: BORDER }}>
            <Pressable
              ...
              disabled={saving}
              style={{ height: 52, borderRadius: 14, backgroundColor: saving ? TEXT_MUTED : BRAND, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 8 }}
            >
              {saving ? <ActivityIndicator color={BRAND_TEXT_ON} size="small" /> : <Feather name="check" size={18} color={BRAND_TEXT_ON} />}
              <Text style={{ color: BRAND_TEXT_ON, fontWeight: '700', fontSize: 16 }}>{saving ? 'Creating…' : `Create Order — $${(totalCents / 100).toFixed(2)}`}</Text>
            </Pressable>
          </View>
        )}
      </View>
