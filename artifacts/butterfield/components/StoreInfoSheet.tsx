...
const STORE_LAT = -33.8349;
const STORE_LNG = 150.9942;
...
export default function StoreInfoSheet({ visible, store, onClose }: Props) {
  ...
  const todayHours   = activeStore.todayHours ?? FALLBACK_HOURS.find(h => h.dayOfWeek === todayDow);
  ...
  const handleDirections = () => {
    ...
    const coords = activeStore.latitude && activeStore.longitude
      ...
      : '';
    Linking.openURL(`https://maps.apple.com/?q=${encodeURIComponent('Butterfield Cookies')}&address=${encodeURIComponent(fullAddress)}${coords}`);
  };
      ...
      >
        ...
        <Pressable style={s.mapWrap} onPress={handleDirections}>
          {(activeStore.latitude && activeStore.longitude) ? (
            <Image
              source={{ uri: staticMapUrl(activeStore.latitude, activeStore.longitude, SCREEN_W) }}
