      ...
      <View style={{ paddingHorizontal: 20, gap: 16, paddingTop: 20 }}>
        <View style={styles.statsRow}>
          {[
            ...
            <View key={s.label} style={[styles.statCard, { backgroundColor: colors.card, borderRadius: 14, borderColor: colors.border, borderWidth: 1 }]}>
              <Text style={[{ color: colors.primary, fontFamily: 'Inter_700Bold', fontSize: 20 }]}>{s.value}</Text>
              <Text style={[{ color: colors.mutedForeground, fontFamily: 'Inter_400Regular', fontSize: 12 }]}>{s.label}</Text>
            </View>
          ))}
        </View>
        ...
        {recentShifts.length > 0 && (
          <View style={[{ backgroundColor: colors.card, borderRadius: 16, padding: 16, gap: 12, borderColor: colors.border, borderWidth: 1 }]}>
            <Text style={[{ color: colors.foreground, fontFamily: 'Inter_600SemiBold', fontSize: 15 }]}>Recent Shifts</Text>
            {recentShifts.map((shift) => (
              <View key={shift.id} style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                {shift.hoursWorked && <Text style={[{ color: colors.primary, fontFamily: 'Inter_700Bold', fontSize: 13 }]}>{shift.hoursWorked}</Text>}
              </View>
            ))}
          </View>
        )}
