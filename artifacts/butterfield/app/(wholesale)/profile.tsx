  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: colors.background }} contentContainerStyle={{ paddingBottom: 120 }} showsVerticalScrollIndicator={false}>
      <LinearGradient colors={['#4B72C4', '#3058A8']} style={[styles.header, { paddingTop: insets.top + 16 }]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}>
        ...
        <Text style={[{ color: 'rgba(255,255,255,0.8)', fontSize: 13, fontFamily: 'Inter_500Medium' }]}>
      ...
      <View style={{ paddingHorizontal: 20, gap: 16, paddingTop: 20 }}>
        {account && (
          <View style={[{ backgroundColor: colors.card, borderRadius: 16, padding: 16, gap: 12, borderColor: colors.border, borderWidth: 1 }]}>
            <Text style={[{ color: colors.foreground, fontFamily: 'Inter_600SemiBold', fontSize: 15 }]}>Account Details</Text>
            {[
              ...
              <View key={row.label} style={{ flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 6, borderBottomWidth: 1, borderBottomColor: colors.border }}>
                <Text style={[{ color: colors.mutedForeground, fontFamily: 'Inter_400Regular', fontSize: 13 }]}>{row.label}</Text>
                <Text style={[{ color: colors.foreground, fontFamily: 'Inter_500Medium', fontSize: 13, maxWidth: '60%', textAlign: 'right' }]}>{row.value}</Text>
              </View>
            ))}
          </View>
        )}
        ...
        {account && (
          <View style={[{ backgroundColor: colors.card, borderRadius: 16, padding: 16, gap: 10, borderColor: colors.border, borderWidth: 1 }]}>
            <Text style={[{ color: colors.foreground, fontFamily: 'Inter_600SemiBold', fontSize: 15 }]}>Credit Utilisation</Text>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
              <Text style={[{ color: colors.mutedForeground, fontFamily: 'Inter_400Regular', fontSize: 13 }]}>${(creditUsed / 100).toFixed(2)} used</Text>
              <Text style={[{ color: colors.primary, fontFamily: 'Inter_700Bold', fontSize: 13 }]}>${(creditLimit / 100).toFixed(2)} limit</Text>
            </View>
            <View style={[{ height: 8, borderRadius: 4, backgroundColor: colors.muted, overflow: 'hidden' }]}>
              <View style={[{ height: '100%', borderRadius: 4, backgroundColor: creditUsed / creditLimit > 0.8 ? '#EF4444' : colors.primary, width: `${Math.min(100, (creditUsed / creditLimit) * 100)}%` }]} />
            </View>
          </View>
        )}

        <View style={[{ backgroundColor: colors.card, borderRadius: 16, padding: 16, gap: 8, borderColor: colors.border, borderWidth: 1 }]}>
          <Text style={[{ color: colors.foreground, fontFamily: 'Inter_600SemiBold', fontSize: 15, marginBottom: 4 }]}>Order Cut-Off Schedule</Text>
          {[
            ...
            <View key={row.delivery} style={{ flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 4 }}>
              <Text style={[{ color: colors.mutedForeground, fontFamily: 'Inter_400Regular', fontSize: 13 }]}>{row.delivery} delivery</Text>
              <Text style={[{ color: colors.primary, fontFamily: 'Inter_600SemiBold', fontSize: 13 }]}>{row.cutOff}</Text>
            </View>
          ))}
        </View>

        <View style={[{ backgroundColor: colors.card, borderRadius: 16, overflow: 'hidden', borderColor: colors.border, borderWidth: 1 }]}>
          {[
            ...
            <Pressable key={item.label} onPress={item.onPress}
              style={[{ flexDirection: 'row', alignItems: 'center', gap: 12, padding: 16 }, i < arr.length - 1 && { borderBottomWidth: 1, borderBottomColor: colors.border }]}>
              <View style={[{ width: 34, height: 34, borderRadius: 10, alignItems: 'center', justifyContent: 'center', backgroundColor: `${colors.primary}15` }]}>
                <Feather name={item.icon as any} size={16} color={colors.primary} />
              </View>
              <Text style={[{ flex: 1, color: colors.foreground, fontFamily: 'Inter_500Medium', fontSize: 15 }]}>{item.label}</Text>
              <Feather name="chevron-right" size={16} color={colors.mutedForeground} />
            </Pressable>
          ))}
        </View>

        <Pressable onPress={handleLogout} style={[{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 10, padding: 16, backgroundColor: colors.card, borderRadius: 16, borderColor: '#DC2626', borderWidth: 1 }]}>
