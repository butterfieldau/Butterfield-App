...
const TABS = ['Store', 'Rewards', 'Notify', 'Managers', 'Homepage'] as const;
...
function HomepageTab() {
  ...
  const { data: specialsData, refetch: refetchSpecials } = useQuery({
    queryKey: ['director-home-specials'],
    queryFn:  () => api.director.homeSpecialsList(),
  });

  const content  = homeData?.data;
  const specials: any[] = specialsData?.data ?? [];

  const [heroBgColor,       setHeroBgColor]       = useState('#40C0F2');
  const [heroImageUrl,      setHeroImageUrl]       = useState('');
  const [heroCampaignTag,   setHeroCampaignTag]    = useState('');
  const [heroCampaignTitle, setHeroCampaignTitle]  = useState('');
  const [heroCampaignSub,   setHeroCampaignSub]    = useState('');
  ...
  const [specialSub,        setSpecialSub]         = useState('');
  ...
  useEffect(() => {
    if (!content) return;
    setHeroBgColor(content.hero?.bgColor         ?? '#40C0F2');
    setHeroImageUrl(content.hero?.imageUrl        ?? '');
    setHeroCampaignTag(content.hero?.campaignTag  ?? '');
    setHeroCampaignTitle(content.hero?.campaignTitle ?? '');
    setHeroCampaignSub(content.hero?.campaignSubtitle ?? '');
    setHeroButtonText(content.hero?.buttonText    ?? '');
    setHeroButtonLink(content.hero?.buttonLink    ?? '');
    setPromoImageUrl(content.promo?.imageUrl      ?? '');
    setPromoTitle(content.promo?.title            ?? '');
    setPromoSubtitle(content.promo?.subtitle      ?? '');
  }, [homeData]);

  const saveHero = async () => {
    setSavingHero(true);
    try {
      await api.director.updateHomeContent({
        hero_bg_color:          heroBgColor,
        hero_image_url:         heroImageUrl  || null,
        hero_campaign_tag:      heroCampaignTag,
        hero_campaign_title:    heroCampaignTitle,
        hero_campaign_subtitle: heroCampaignSub,
        hero_button_text:       heroButtonText,
        hero_button_link:       heroButtonLink,
      });
      await qc.invalidateQueries({ queryKey: ['home-content'] });
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
      Alert.alert('Saved', 'Hero banner updated. Customers will see it immediately.');
    } catch (e: any) {
  ...
  const savePromo = async () => {
    ...
    try {
      await api.director.updateHomeContent({
        ...
        promo_subtitle:  promoSubtitle || null,
      });
      await qc.invalidateQueries({ queryKey: ['home-content'] });
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
      Alert.alert('Saved', 'Promo section updated.');
    } catch (e: any) {
      Alert.alert('Error', e.message ?? 'Could not save.');
  ...
  const addSpecial = async () => {
    ...
    try {
      await api.director.homeSpecialsCreate({
        title:     specialTitle.trim(),
        subtitle:  specialSub   || undefined,
        imageUrl:  specialImage || undefined,
        badgeText: specialBadge || undefined,
      });
      refetchSpecials();
      setSpecialTitle(''); setSpecialSub(''); setSpecialImage(''); setSpecialBadge('');
  ...
  const deleteSpecial = (id: string) => {
    Alert.alert('Remove special', 'Remove this special from the home screen?', [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Remove', style: 'destructive',
        onPress: async () => {
          try {
            await api.director.homeSpecialsDelete(id);
            refetchSpecials();
          } catch (e: any) { Alert.alert('Error', e.message); }
        },
      },
  ...
  const toggleSpecial = async (id: string, isActive: boolean) => {
    try {
      await api.director.homeSpecialsUpdate(id, { isActive });
      refetchSpecials();
    } catch (e: any) { Alert.alert('Error', e.message); }
  };
  ...
  return (
    <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 16, gap: 16, paddingBottom: 100 }} showsVerticalScrollIndicator={false}>
      ...
      {/* ── Hero banner ── */}
      <Text style={styles.section}>HERO BANNER</Text>
      <View style={[styles.card, { backgroundColor: CARD, borderColor: BORDER }]}>
        <View style={[styles.infoBanner, { backgroundColor: '#EBF8FF', borderColor: BLUE + '40' }]}>
          <Feather name="image" size={13} color={BLUE} />
          <Text style={[styles.infoBannerText, { color: BLUE }]}>
            This is the full-width banner at the top of the customer home screen. Change it any time to run a campaign or promote a special.
          </Text>
        </View>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
          <View style={{ width: 34, height: 34, borderRadius: 8, backgroundColor: heroBgColor || '#40C0F2', borderWidth: 1, borderColor: BORDER }} />
          <View style={{ flex: 1, gap: 5 }}>
            <Text style={styles.fieldLabel}>Background colour (hex)</Text>
            <TextInput
              style={[styles.input, { borderColor: BORDER, color: TEXT }]}
              value={heroBgColor}
              onChangeText={setHeroBgColor}
              placeholder="#40C0F2"
              placeholderTextColor={MUTED}
              autoCapitalize="none"
              autoCorrect={false}
        ...
        <View style={{ gap: 5 }}>
          <Text style={styles.fieldLabel}>Background image URL (optional — shown behind text)</Text>
          <TextInput style={[styles.input, { borderColor: BORDER, color: TEXT }]} value={heroImageUrl} onChangeText={setHeroImageUrl}
            placeholder="https://your-image-url.com/banner.jpg" placeholderTextColor={MUTED} autoCapitalize="none" keyboardType="url" autoCorrect={false} />
        </View>
        <View style={[styles.divider, { backgroundColor: BORDER }]} />
        <View style={{ gap: 5 }}>
          <Text style={styles.fieldLabel}>Campaign label (small tag above headline)</Text>
          <TextInput style={[styles.input, { borderColor: BORDER, color: TEXT }]} value={heroCampaignTag} onChangeText={setHeroCampaignTag}
            placeholder="🍪 DAILY SPECIAL" placeholderTextColor={MUTED} />
        </View>
        <View style={{ gap: 5 }}>
          <Text style={styles.fieldLabel}>Campaign headline</Text>
          <TextInput style={[styles.input, { borderColor: BORDER, color: TEXT }]} value={heroCampaignTitle} onChangeText={setHeroCampaignTitle}
            placeholder="Cookie & Cream Sandwich" placeholderTextColor={MUTED} />
        </View>
        <View style={{ gap: 5 }}>
          <Text style={styles.fieldLabel}>Campaign subtitle</Text>
          <TextInput style={[styles.input, { borderColor: BORDER, color: TEXT }]} value={heroCampaignSub} onChangeText={setHeroCampaignSub}
            placeholder="Two warm cookies + vanilla cream" placeholderTextColor={MUTED} />
        </View>
        <View style={[styles.divider, { backgroundColor: BORDER }]} />
        <View style={{ gap: 5 }}>
          <Text style={styles.fieldLabel}>Button text</Text>
          <TextInput style={[styles.input, { borderColor: BORDER, color: TEXT }]} value={heroButtonText} onChangeText={setHeroButtonText}
            placeholder="Order now" placeholderTextColor={MUTED} />
        </View>
        <View style={{ gap: 5 }}>
          <Text style={styles.fieldLabel}>Button destination</Text>
          <TextInput style={[styles.input, { borderColor: BORDER, color: TEXT }]} value={heroButtonLink} onChangeText={setHeroButtonLink}
            placeholder="menu" placeholderTextColor={MUTED} autoCapitalize="none" autoCorrect={false} />
          <Text style={[styles.hint, { color: MUTED }]}>Options: menu · loyalty · cart · orders</Text>
        </View>
        <Pressable
          onPress={saveHero}
          disabled={savingHero}
          style={[styles.saveBtn, { backgroundColor: BLUE, opacity: savingHero ? 0.7 : 1 }]}
        >
          {savingHero
            ? <ActivityIndicator color="#fff" size="small" />
            : <Text style={styles.saveBtnText}>Save Hero Banner</Text>}
        </Pressable>
      </View>

      {/* ── Promo section ── */}
      <Text style={styles.section}>PROMO SECTION</Text>
      <View style={[styles.card, { backgroundColor: CARD, borderColor: BORDER }]}>
        <View style={[styles.infoBanner, { backgroundColor: '#F5F3FF', borderColor: '#8B5CF6' + '40' }]}>
          <Feather name="layers" size={13} color="#8B5CF6" />
          <Text style={[styles.infoBannerText, { color: '#8B5CF6' }]}>
            This appears below the loyalty bar as a large image + text card. Drop in any promotional image and copy.
          </Text>
        </View>
        <View style={{ gap: 5 }}>
          <Text style={styles.fieldLabel}>Promo image URL</Text>
          <TextInput style={[styles.input, { borderColor: BORDER, color: TEXT }]} value={promoImageUrl} onChangeText={setPromoImageUrl}
            placeholder="https://your-image-url.com/promo.jpg" placeholderTextColor={MUTED} autoCapitalize="none" keyboardType="url" autoCorrect={false} />
        </View>
        <View style={{ gap: 5 }}>
          <Text style={styles.fieldLabel}>Promo headline</Text>
          <TextInput style={[styles.input, { borderColor: BORDER, color: TEXT }]} value={promoTitle} onChangeText={setPromoTitle}
            placeholder="Double Rewards this week" placeholderTextColor={MUTED} />
        </View>
        <View style={{ gap: 5 }}>
          <Text style={styles.fieldLabel}>Promo description</Text>
          <TextInput style={[styles.input, { borderColor: BORDER, color: TEXT }]} value={promoSubtitle} onChangeText={setPromoSubtitle}
            placeholder="Earn double loyalty points on all orders" placeholderTextColor={MUTED} />
        </View>
        <Pressable
          onPress={savePromo}
          disabled={savingPromo}
          style={[styles.saveBtn, { backgroundColor: '#8B5CF6', opacity: savingPromo ? 0.7 : 1 }]}
        >
          {savingPromo
            ? <ActivityIndicator color="#fff" size="small" />
            : <Text style={styles.saveBtnText}>Save Promo Section</Text>}
        </Pressable>
      </View>

      {/* ── Current specials ── */}
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
        <Text style={styles.section}>CURRENT SPECIALS</Text>
        <Pressable
          onPress={() => setShowAddSpecial(!showAddSpecial)}
          style={{ flexDirection: 'row', alignItems: 'center', gap: 5, padding: 8 }}
        >
          <Feather name={showAddSpecial ? 'x' : 'plus-circle'} size={16} color={GREEN} />
          <Text style={{ color: GREEN, fontFamily: 'Inter_600SemiBold', fontSize: 13 }}>
            {showAddSpecial ? 'Cancel' : 'Add special'}
          </Text>
        </Pressable>
      </View>

      {showAddSpecial && (
        <View style={[styles.card, { backgroundColor: CARD, borderColor: GREEN + '60', borderWidth: 2 }]}>
          <Text style={[styles.rowTitle, { marginBottom: 2 }]}>New Special</Text>
          <View style={{ gap: 5 }}>
            <Text style={styles.fieldLabel}>Title *</Text>
            <TextInput style={[styles.input, { borderColor: BORDER, color: TEXT }]} value={specialTitle} onChangeText={setSpecialTitle}
              placeholder="e.g. Buy 2 Get 1 Free Cookies" placeholderTextColor={MUTED} />
          </View>
          <View style={{ gap: 5 }}>
            <Text style={styles.fieldLabel}>Description</Text>
            <TextInput style={[styles.input, { borderColor: BORDER, color: TEXT }]} value={specialSub} onChangeText={setSpecialSub}
              placeholder="Available this weekend only" placeholderTextColor={MUTED} />
          </View>
          <View style={{ gap: 5 }}>
            <Text style={styles.fieldLabel}>Image URL (optional)</Text>
            <TextInput style={[styles.input, { borderColor: BORDER, color: TEXT }]} value={specialImage} onChangeText={setSpecialImage}
              placeholder="https://..." placeholderTextColor={MUTED} autoCapitalize="none" keyboardType="url" autoCorrect={false} />
          </View>
          <View style={{ gap: 5 }}>
            <Text style={styles.fieldLabel}>Badge label (optional — e.g. TODAY ONLY)</Text>
            <TextInput style={[styles.input, { borderColor: BORDER, color: TEXT }]} value={specialBadge} onChangeText={setSpecialBadge}
              placeholder="LIMITED TIME" placeholderTextColor={MUTED} />
          </View>
          <Pressable
            onPress={addSpecial}
            disabled={savingSpecial || !specialTitle.trim()}
            style={[styles.saveBtn, { backgroundColor: GREEN, opacity: savingSpecial || !specialTitle.trim() ? 0.5 : 1 }]}
          >
            {savingSpecial
              ? <ActivityIndicator color="#fff" size="small" />
              : <Text style={styles.saveBtnText}>Add Special</Text>}
          </Pressable>
        </View>
      )}
      ...
      {specials.length === 0 ? (
        <View style={{ alignItems: 'center', paddingVertical: 24 }}>
          <Feather name="star" size={28} color={MUTED} />
          <Text style={[styles.emptyText, { marginTop: 8 }]}>No specials on the home screen yet.</Text>
          <Text style={[styles.emptyText, { color: MUTED + 'AA', fontSize: 12, marginTop: 4 }]}>Tap "Add special" to create one.</Text>
        </View>
      ) : (
        specials.map((s: any) => (
          <View key={s.id} style={[styles.card, { backgroundColor: CARD, borderColor: BORDER }]}>
            <View style={{ flexDirection: 'row', alignItems: 'flex-start', gap: 10 }}>
              <View style={{ flex: 1, gap: 3 }}>
                <Text style={styles.rowTitle}>{s.title}</Text>
                {s.subtitle ? <Text style={styles.rowSub}>{s.subtitle}</Text> : null}
                {s.badgeText ? (
                  <View style={{ flexDirection: 'row', marginTop: 2 }}>
                    <View style={{ backgroundColor: AMBER + '22', paddingHorizontal: 8, paddingVertical: 2, borderRadius: 8 }}>
                      <Text style={{ fontSize: 10, fontFamily: 'Inter_700Bold', color: AMBER, letterSpacing: 0.6 }}>{s.badgeText}</Text>
                    </View>
                  </View>
                ) : null}
              </View>
              <Switch
                value={s.isActive}
                onValueChange={(v) => toggleSpecial(s.id, v)}
            ...
            <View style={[styles.divider, { backgroundColor: BORDER }]} />
            <Pressable
              onPress={() => deleteSpecial(s.id)}
              style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}
            >
              <Feather name="trash-2" size={14} color={RED} />
              <Text style={{ color: RED, fontSize: 13, fontFamily: 'Inter_600SemiBold' }}>Remove from home screen</Text>
            </Pressable>
          </View>
        ))
      )}
