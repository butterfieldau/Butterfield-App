    ...
    <View style={[cs.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
      ...
      <View style={{ padding: 14, gap: 10 }}>
        ...
        <View style={{ flexDirection: 'row', gap: 8, marginTop: 2 }}>
          {store.pickupAvailable   && <View style={cs.serviceChip}><Feather name="shopping-bag" size={11} color="#40C0F2" /><Text style={[cs.serviceText, { color: '#40C0F2' }]}>Pickup</Text></View>}
        ...
        <View style={{ flexDirection: 'row', gap: 8, marginTop: 4 }}>
          {store.latitude && store.longitude && (
            <Pressable style={[cs.actionBtn, { flex: 1 }]} onPress={handleDirections}>
              <Feather name="map" size={14} color="#40C0F2" />
          ...
          {store.pickupAvailable && store.status === 'open' && (
            <Pressable style={[cs.actionBtn, { flex: 1, backgroundColor: '#40C0F2' }]} onPress={() => router.push('/(customer)/menu')}>
      ...
      <View style={{ paddingHorizontal: 16, paddingTop: 16, gap: 12 }}>
        {isLoading ? (
          <View style={{ paddingVertical: 60, alignItems: 'center' }}>
            <ActivityIndicator size="large" color="#40C0F2" />
          </View>
        ) : stores.length === 0 ? (
